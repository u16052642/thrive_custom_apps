...
class FormioPublicController(http.Controller):
    ...
    def form_config(self, form_uuid):
        ...
        if form and form.builder_id.schema:
            ...
            etl_thrive_config = form.builder_id.sudo()._etl_thrive_config(
                formio_form=form, params=args.to_dict()
            )
            res['options'].update(etl_thrive_config.get('options', {}))
    ...
    def public_form_submission(self, uuid):
        ...
        # ETL thrive data
        if form:
            etl_thrive_data = form.sudo()._etl_thrive_data()
            submission_data.update(etl_thrive_data)
    ...
    def public_form_new_config(self, builder_uuid, **kwargs):
        ...
        args = request.httprequest.args
        etl_thrive_config = formio_builder.sudo()._etl_thrive_config(params=args.to_dict())
        res['options'].update(etl_thrive_config.get('options', {}))
    ...
    def public_form_new_submission(self, builder_uuid, **kwargs):
        ...
        args = request.httprequest.args
        submission_data = {}
        etl_thrive_data = formio_builder.sudo()._etl_thrive_data(params=args.to_dict())
        submission_data.update(etl_thrive_data)
        return request.make_json_response(submission_data)
