...
from thrive.addons.connector.components.mapper import mapping, only_create
...
class WooSaleOrderImportMapper(Component):
    ...
    def _prepare_lines(
        ...
        for tax in taxes:
            ...
            if woo_tax and woo_tax.thrive_id:
                tax_records.append(woo_tax.thrive_id.id)
            if woo_tax and not woo_tax.thrive_id:
                search_conditions = [
...
class WooSaleOrderLineImportMapper(Component):
    ...
    def fetch_list_of_tax(self, taxes, tax_lines):
        ...
        for tax in taxes:
            ...
            if woo_tax and woo_tax.thrive_id:
                result.append(woo_tax.thrive_id.id)
                continue
            rate_id = tax.get("id")
            tax_line = next(
