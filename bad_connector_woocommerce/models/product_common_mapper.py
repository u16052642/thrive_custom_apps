...
class ProductCommonImportMapper(Component):
    ...
    def categ_id(self, record):
        ...
        for category in record.get("categories", []):
            ...
            if woo_binding and woo_binding.thrive_id:
                category_id = woo_binding.thrive_id.id
                break
        ...
        for option in options:
            ...
            if not product_attribute_value:
                ...
                self.env["woo.product.attribute.value"].create(
                    {
                        "name": option,
                        "attribute_id": product_attribute.thrive_id.id,
                        "woo_attribute_id": product_attribute.id,
                        "backend_id": self.backend_record.id,
                        "external_id": created_id,
                        "thrive_id": attribute_value.id if attribute_value else None,
                    }
                )
        return True
