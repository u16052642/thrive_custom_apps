...
class TestImportRefund(BaseWooTestCase):
    ...
    def test_import_order_for_refund(self):
        ...
        sale_order_thrive = sale_order1.thrive_id
        sale_order_thrive.action_confirm()
        delivery_order = sale_order_thrive.picking_ids
        ...
        sale_order_line = sale_order_thrive.order_line.filtered(
            lambda line: line.product_id == delivery_order.move_ids[0].product_id
        )
        sale_order_line.product_id.qty_available = 2
        sale_order_thrive.picking_ids[0].move_ids[0].quantity = 2
        sale_order_thrive.picking_ids[0].button_validate()
        self.assertEqual(
            sale_order_thrive.picking_ids[0].state,
            "done",
            "Picking state should be done!",
        )
        self.assertEqual(
            sale_order_thrive.picking_ids[1].state,
            "done",
            "Picking state should be done!",
        )
        ...
        return_picking = sale_order_thrive.picking_ids.filtered(
        ...
        self.assertEqual(
            sale_order_thrive.woo_order_status_id.code,
            "refunded",
            "Sale Order is Not in 'Refunded' state in WooCommerce.",
        )
        self.assertEqual(len(sale_order_thrive.picking_ids), 4)
