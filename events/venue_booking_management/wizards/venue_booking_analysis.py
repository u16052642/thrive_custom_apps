...
from thrive.tools import date_utils, io, xlsxwriter
...
class VenueBookingAnalysis(models.TransientModel):
    ...
    def action_print_xlsx(self):
        ...
        data = {
            ...
            'today_date': current,
            'company': [rec.partner_id.name, rec.street, rec.logo],
            'form': self.read()[0],
            'venue': venue
        }
        return {
    ...
    def get_xlsx_report(self, data, response):
        ...
        txt_head = workbook.add_format(
            {'bold': True, 'font_size': '12px', 'border': 2})
        ...
        for i in range(0, len(rec)):
            ...
            if rec[i]['booking_type'] == 'day':
                ...
                sheet.merge_range('H%d:I%d' % (j + 1, j + 1),
                                  rec[i]['booking_type'], txt)
            sheet.merge_range('J%d:L%d' % (j + 1, j + 1), rec[i]['name'], txt)
            sheet.merge_range('M%d:N%d' % (j + 1, j + 1),
                              fields.Datetime.to_string(rec[i]['start_date']),
                              txt)
            sheet.merge_range('O%d:P%d' % (j + 1, j + 1),
                              fields.Datetime.to_string(rec[i]['end_date']),
                              txt)
            if rec[i]['state'] == 'draft':
                ...
                sheet.merge_range('Q%d:R%d' % (j + 1, j + 1), rec[i]['state'],
                                  txt)
            j += 1
            k += 1
        workbook.close()
