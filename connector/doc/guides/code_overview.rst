...
.. code-block:: python
    ...
    class PartnerImporter(Component):
        ...
        def run(self, external_id):
            ...
            # this one knows how to convert magento data to thrive data
            mapper = self.component(usage='import.mapper')
            # this one knows how to link magento/thrive records
            binder = self.component(usage='binder')

            # read external data from magento
            external_data = backend_adapter.read(external_id)
            # convert to thrive data
            internal_data = mapper.map_record(external_data).values()
            # find if the magento id already exists in thrive
