...
class Binder(AbstractComponent):
    ...
    _thrive_field = "thrive_id"  # override in sub-classes
    ...
    def to_internal(self, external_id, unwrap=False):
        ...
        if not bindings:
            if unwrap:
                return self.model.browse()[self._thrive_field]
            return self.model.browse()
        bindings.ensure_one()
        if unwrap:
            bindings = bindings[self._thrive_field]
    ...
    def unwrap_binding(self, binding):
        ...
        return binding[self._thrive_field]
    ...
    def unwrap_model(self):
        ...
        """
        try:
            column = self.model._fields[self._thrive_field]
        except KeyError as err:
            raise ValueError(
                f"Cannot unwrap model {self.model._name}, because it has no {self._thrive_field} fields"
            ) from err
