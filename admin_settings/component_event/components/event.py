...
For the creation of a record, it is in :meth:`thrive.models.BaseModel.create`.
...
from thrive.addons.component.core import AbstractComponent, Component
...
class EventCollecter(Component):
    ...
    :class:`thrive.addons.component_event.core.EventWorkContext` class should be
    used for this Component, because it can work
    without a collection.

    It is used by :meth:`thrive.addons.component_event.models.base.Base._event`.
    ...
    def _complete_component_build(cls):
        ...
        # dynamically rebuild when thrive registry is rebuild, we
        # are sure that the result is always the same for a lookup
        # until the next rebuild of thrive's registry
