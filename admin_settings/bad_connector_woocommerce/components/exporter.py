...
from thrive.addons.component.core import AbstractComponent
from thrive.addons.connector.exception import IDMissingInBackend
from thrive.addons.queue_job.job import identity_exact
...
class WooExporter(AbstractComponent):
    ...
    def __init__(self, work_context):
        ...
        # the latest status in thrive instead of sending the import request again
        self.response_data = None
        self.remote_record = None
        self.thrive_record = None
    ...
    def create_get_binding(self, extra_data=None):
        ...
        record = self.thrive_record.with_context(active_test=False)
        external_id = False
        if self._default_binding_field and record[self._default_binding_field]:
            binding = record[self._default_binding_field][:1]
            external_id = binding[self.backend_adapter._thrive_ext_id_key]
    ...
    def run(self, binding, record=None, fields=None, *args, **kwargs):
        ...
        if not record and binding:
            record = binding.thrive_id

        assert record
        self.thrive_record = record
        ...
        **kwargs,
        ...
        if component_usage == "record.importer":
            ...
            if (
                ...
                external_id = binding[exporter.backend_adapter._thrive_ext_id_key]
    ...
    def _run(self, fields=None, **kwargs):
        ...
        # Check before calling export dependencies as we need self.thrive_record
        if not self.binding and not self.thrive_record:
            raise ValidationError(_("No record found to export!!!"))
        if not self.thrive_record:
            self.thrive_record = self.binding.thrive_id
        ...
        if self.external_id:
            ...
            if isinstance(res, dict):
                ...
                self.external_id = (
                    self.binding[self.backend_adapter._thrive_ext_id_key] or None
                )
            # BAD end
