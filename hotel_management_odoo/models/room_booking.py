...
from thrive.tools.safe_eval import pytz
        ...
        if tz_name:
            ...
            for rec in check_outs:
                ...
                staff = self.env['res.users'].search_count(
                    [('groups_id', 'in',
                      [self.env.ref('hotel_management_thrive.hotel_group_admin').id,
                       self.env.ref(
                           'hotel_management_thrive.cleaning_team_group_head').id,
                       self.env.ref(
                           'hotel_management_thrive.cleaning_team_group_user').id,
                       self.env.ref(
                           'hotel_management_thrive.hotel_group_reception').id,
                       self.env.ref(
                           'hotel_management_thrive.maintenance_team_group_leader').id,
                       self.env.ref(
                           'hotel_management_thrive.maintenance_team_group_user').id
                       ])])
