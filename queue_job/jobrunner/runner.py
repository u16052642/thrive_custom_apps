...
* Or, if using ``anybox.recipe.thrive``, add this to your buildout configuration:

.. code-block:: ini

  [thrive]
  recipe = anybox.recipe.thrive
  (...)
...
* Or, if using ``anybox.recipe.thrive``:
...
* Confirm the runner is starting correctly by checking the thrive log file:
...
#
# On the other hand, the thrive configuration file can be extended at will,
...
def _connection_info_for(db_name):
    db_or_uri, connection_info = thrive.sql_db.connection_info_for(db_name)
...
class Database:
    ...
    def _has_queue_job(self):
        with closing(self.conn.cursor()) as cr:
            ...
            if not cr.fetchone():
                _logger.debug("%s doesn't seem to be an thrive db", self.db_name)
                return False
            cr.execute(
...
class QueueJobRunner:
    ...
    def wait_notification(self):
        ...
        if not wakeup_time:
            ...
            timeout = wakeup_time - _thrive_now()
