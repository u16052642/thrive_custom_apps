...
from thrive.tests.case import TestCase as _TestCase
from thrive.tests.common import MetaCase

from thrive.addons.queue_job.delay import Graph

# pylint: disable=thrive-addons-relative-import
from thrive.addons.queue_job.job import Job
...
def trap_jobs():
    ...
    * the execution of the job itself in a second test
    ...
    """
    with mock.patch(
        "thrive.addons.queue_job.delay.Job",
        name="Job Class",
        auto_spec=True,
        unsafe=True,
    ) as job_cls_mock:
...
def mock_with_delay():
    ...
    * the second to check which job method was called and with which arguments.
    ...
    """
    with mock.patch(
        "thrive.addons.queue_job.models.base.DelayableRecordset",
...
def load_doctests(module):
    ...
    def load_tests(loader, tests, ignore):
        ...
        for idx, test in enumerate(doctest.DocTestSuite(module)):
            thrive_test = OdooDocTestCase(test, seq=idx)
            thrive_test.test_tags = {"standard", "at_install", "queue_job", "doctest"}
            tests.addTest(thrive_test)
