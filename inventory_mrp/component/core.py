...
from thrive.tools import LastOrderedSet, OrderedSet
...
# this is duplicated from thrive.models.MetaModel._get_addon_name() which we
# unfortunately can't use because it's an instance method and should have been
# a @staticmethod
def _get_addon_name(full_name):
    # The (Odoo) module name can be in the ``thrive.addons`` namespace
    # or not. For instance, module ``sale`` can be imported as
    # ``thrive.addons.sale`` (the right way) or ``sale`` (for backward
    # compatibility).
    module_parts = full_name.split(".")
    if len(module_parts) > 2 and module_parts[:2] == ["thrive", "addons"]:
...
# it will be cleared and updated when the thrive's registry is rebuilt
...
class WorkContext:
    ...
    .. attribute:: model
        ...
        :class:`~thrive.api.Environment` than the ``collection`` attribute.
    ...
    def component_by_name(self, name, model_name=None):
        ...
        A :exc:`thrive.addons.component.exception.NoComponentError` is raised
        if:
    ...
    def component(self, usage=None, model_name=None, **kw):
        ...
        A :exc:`thrive.addons.component.exception.SeveralComponentError` is
...
class AbstractComponent(metaclass=MetaComponent):
    ...
    * It is advised to namespace the names of the components (e.g.
    ...
    def _build_component(cls, registry):
        ...
        :meth:`thrive.addons.connector.components.mapper.Mapper._complete_component_build`
